...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch serie", details: errorMessage },
      { status: 500 }
    );
  }
}
...
async function uploadImage(
  ...
  const finalExtension = allowedExtensions.includes(extension)
    ? extension
    : "jpg";
...
async function deleteImageFile(imagePath: string | null) {
  ...
  if (
    ...
    imagePath.startsWith("/backdrops/")
  ) {
    try {
      ...
      if (
        ...
        console.warn(
          `[API deleteImageFile] Could not delete ${imagePath}:`,
...
export async function PUT(
  ...
  console.log(
    `[API PUT /api/series/${serieIdFromContext}] Received update request.`
  );
  ...
  try {
    ...
    if (!serieId) {
      ...
      return NextResponse.json(
        { error: "ID de serie no proporcionado" },
        { status: 400 }
      );
    }
    ...
    console.log(
      "[API PUT] FormData received:",
      Array.from(apiFormData.entries())
    );
    ...
    const updateData: Partial<Prisma.SerieUpdateInput> = {
      ...
      estado: apiFormData.get("estado") as EstadoSerie,
      ...
      fechaEstreno: apiFormData.get("fechaEstreno")
        ? new Date(apiFormData.get("fechaEstreno") as string)
        : null,
    ...
    if (!serieActual) {
      console.error(
        `[API PUT /api/series/${serieId}] Serie not found for update.`
      );
      return NextResponse.json(
      ...
      if (value !== null && value !== undefined) {
        if (typeof value === "string") {
          if (
            key === "año" ||
            key === "temporadas" ||
            key === "duracionPromedio"
          ) {
            const numVal = parseInt(value, 10);
            if (!isNaN(numVal)) updateData[key] = numVal;
            else if (value === "") updateData[key] = null;
          } else if (key === "rating") {
            const floatVal = parseFloat(value);
            if (!isNaN(floatVal)) updateData[key] = floatVal;
            else if (value === "") updateData[key] = null;
          } else if (key === "estado") {
    ...
    if (posterFile instanceof File && posterFile.size > 0) {
      ...
      updateData.poster = await uploadImage(
        posterFile,
        "poster",
        serieTitleForImage
      );
    ...
    if (bannerFile instanceof File && bannerFile.size > 0) {
      ...
      updateData.banner = await uploadImage(
        bannerFile,
        "banner",
        serieTitleForImage
      );
    ...
    const submittedGenreIds = apiFormData
      .getAll("genreIds[]")
      .map((id) => String(id));
    const submittedTagIds = apiFormData
      .getAll("tagIds[]")
      .map((id) => String(id));
    ...
    if (apiFormData.has("genreIds[]") && submittedGenreIds.length > 0) {
      ...
      const missingGenres = submittedGenreIds.filter(
        (id) => !existingGenres.some((g) => g.id === id)
      );
      if (missingGenres.length > 0) {
        console.error(
          `[API PUT] Géneros no encontrados: ${missingGenres.join(", ")}`
        );
        return NextResponse.json(
      ...
      updateData.generos = {
        set: submittedGenreIds.map((generoId) => ({
          serieId_generoId: { serieId, generoId },
        })),
      };
    ...
    if (apiFormData.has("tagIds[]") && submittedTagIds.length > 0) {
      ...
      const missingTags = submittedTagIds.filter(
        (id) => !existingTags.some((t) => t.id === id)
      );
      if (missingTags.length > 0) {
        console.error(
          `[API PUT] Etiquetas no encontradas: ${missingTags.join(", ")}`
        );
        return NextResponse.json(
      ...
      updateData.tags = {
        set: submittedTagIds.map((tagId) => ({
          serieId_tagId: { serieId, tagId },
        })),
      };
      console.log("[API PUT] Setting tags to:", updateData.tags);
    }

    if (apiFormData.has("actores")) {
      const actoresData = JSON.parse(apiFormData.get("actores") as string);

      // Desconectar actores actuales
      await prisma.serie.update({
        where: { id: serieId },
        data: { actores: { set: [] } },
      });

      // Conectar nuevos actores
      for (const actorData of actoresData) {
        await prisma.actorSerie.create({
          data: {
            serieId: serieId,
            actorId: actorData.actorId,
            personaje: actorData.personaje,
            tipoRol: actorData.tipoRol,
          },
        });
      }
    }
    ...
    console.log(
      `[API PUT /api/series/${serieId}] Final updateData object before Prisma call:`,
      updateData
    );
    ...
    console.log(
      `[API PUT /api/series/${serieId}] Serie updated successfully:`,
      updatedSerie
    );
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      console.error(
        `[API PUT] Error de Prisma: ${error.message}, Código: ${error.code}`
      );
      if (error.code === "P2025") {
        return NextResponse.json(
          {
            error:
              "Uno o más géneros o etiquetas no existen en la base de datos",
          },
          { status: 400 }
        );
      }
    ...
    console.error(
      `[API PUT /api/series/${serieIdFromContext}] Error al actualizar la serie:`,
      error
    );
    return NextResponse.json(
      {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!createData.titulo) {
      ...
      return NextResponse.json(
        { error: "'titulo' es un campo requerido y no puede estar vacío." },
        { status: 400 }
      );
    }
    ...
    if (posterFile instanceof File && posterFile.size > 0) {
      ...
      createData.poster = await uploadImage(
        posterFile,
        "poster",
        createData.titulo
      );
    ...
    if (bannerFile instanceof File && bannerFile.size > 0) {
      ...
      createData.banner = await uploadImage(
        bannerFile,
        "banner",
        createData.titulo
      );
    ...
    console.log(
      "[API POST] Final createData object before Prisma call:",
    ...
    console.error(
      "[API POST /api/series] Error processing POST request:",
      error,
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      statusCode = 400;
      console.error(
        `[API POST] Prisma Known Error: Code ${errorCode}, Message: ${error.message}`
      );
    }
    return NextResponse.json(
      {
...
export async function DELETE(
  ...
  const { id: serieIdFromContext } = await context.params;
  console.log(
    `[API DELETE /api/series/${serieIdFromContext}] Received delete request.`
  );
  try {
    ...
    if (!serie) {
      console.error(
        `[API DELETE /api/series/${id}] Serie not found for deletion.`
      );
      return NextResponse.json({ error: "Serie not found" }, { status: 404 });
    }
    ...
    console.error(
      `[API DELETE /api/series/${serieIdFromContext}] Error processing DELETE request:`,
      error,
      typeof error === "object" && error !== null && "stack" in error
        ? (error as { stack?: string }).stack
        : undefined
